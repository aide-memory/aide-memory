...
aide-memory is an auto-captured, auto-recalled, path-scoped memory layer for AI coding agents (Claude Code + Cursor today). It runs locally, plugs in via hooks + an MCP server, and uses git for team sync. Sits alongside `CLAUDE.md` and `.cursorrules`; doesn't replace them.
...
- Local-first: SQLite cache + JSON files on your disk. Memory content stays on your machine
