...
**Status: rule template only in 0.5.0.** aide-memory's Windsurf adapter
ships a curated rule template (`src/templates/rules/shared/body.md` +
Windsurf-specific frontmatter) but does **not** yet generate hook or MCP
config files at `aide-memory init`.

## What works today

- If you add aide-memory as an MCP server manually in Windsurf's MCP
...
- Until the adapter ships, see the
  ...
  Windsurf today can run the seven MCP tools (manual setup) and consume
