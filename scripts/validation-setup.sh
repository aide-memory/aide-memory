...
aide-memory remember "Use dayjs not moment for date handling" --layer preferences --scope "src/**"
aide-memory remember "API rate limiting is 100 req/min per user" --layer area_context --scope "src/api/**"
aide-memory remember "All API responses use camelCase" --layer guidelines --scope "src/api/**"
...
aide-memory remember "API handlers return ISO 8601 timestamps" --layer guidelines --scope "src/api/**"
