...
Each task below has step-by-step instructions. Tasks marked **AGENT-ASSISTED** can be partially automated by spinning off an agent with the provided prompt.

---

#### P0.1 — Domain Registration (HUMAN ONLY)
1. Go to [Cloudflare Registrar](https://dash.cloudflare.com) → Domain Registration
2. Search for `aide-memory.dev` (preferred) or `useaide.dev`
3. Purchase (~$10-15/year) and configure DNS
4. HTTPS is automatic with Cloudflare
5. Set up email forwarding: `hello@aide-memory.dev` → your personal email
- [ ] Domain registered
- [ ] DNS configured
- [ ] Email forwarding active

#### P0.2 — Legal (HUMAN ONLY)
1. **Trademark search:** Go to [tess2.uspto.gov](https://tess2.uspto.gov) → Basic Word Mark Search → search "AIDE" in International Classes 009 (software) and 042 (SaaS). Check for conflicts with AiDE(R) enterprise AI platform.
2. **EULA:** Draft proprietary freeware EULA. Key clauses: free to use, no modification/redistribution, no warranty, data ownership (user owns their data). Use [GitHub ToS](https://docs.github.com/en/site-policy/github-terms/github-terms-of-service) as structure reference.
3. **Terms & Conditions:** Standard website terms for the landing page. Can use a generator like [TermsFeed](https://www.termsfeed.com) as starting point.
4. **Company registration:** Decide whether to register an LLC for liability/IP protection. Can defer to Phase 2 if not pursuing revenue yet.
- [ ] Trademark search completed (no blocking conflicts)
- [ ] EULA drafted
- [ ] Terms & Conditions drafted
- [ ] Company registration decision made

#### P0.3 — Public GitHub Repo (AGENT-ASSISTED)
The repo creation itself is manual, but the README and docs can be generated by an agent.

**Manual steps:**
1. Create `aide-memory` org on GitHub: [github.com/organizations/new](https://github.com/organizations/new)
2. Create `aide-memory` repo (public)
3. Set up issue templates and release workflow
...
#### P0.4 — npm Package Reservation (HUMAN ONLY)
1. `npm login` (create account at [npmjs.com](https://www.npmjs.com) if needed)
2. From the repo: `cd /Users/meky/code/aide-v0`
3. Test: `cp package.aide-memory.json package.json && npm pack --dry-run` (verify contents)
4. Reserve name: `npm publish --access public` with v0.0.1 (or `npm owner add <your-username> aide-memory`)
5. Restore: `git checkout package.json`
- [ ] npm account ready
- [ ] `aide-memory` name reserved on npm
- [ ] Publish workflow tested

#### P0.5 — Landing Page (AGENT-ASSISTED)
The scaffolding and deployment are manual, but content can be generated by an agent.

**Manual steps:**
1. `npx create-next-app aide-memory-site -e https://github.com/shuding/nextra-docs-template`
2. Deploy to Vercel: [vercel.com/new](https://vercel.com/new) → connect GitHub repo → auto-deploy
3. Point domain DNS to Vercel (CNAME record)
...
#### P1.9 — Cursor Validation (HUMAN ONLY)
...
#### P1.17 — Pre-ship Validation (HUMAN ONLY — 5 scenarios)
...
#### P1.18 — Plugin/Marketplace (HUMAN ONLY)
1. **Claude Code:** Check [Claude Code docs](https://docs.anthropic.com/en/docs/claude-code) for plugin/marketplace submission process. Submit if available.
2. **Cursor:** Check [Cursor marketplace](https://cursor.com/marketplace) for MCP tool listings. Submit if supported.
- [ ] Claude Code listing submitted (or noted as not yet available)
- [ ] Cursor listing submitted (or noted as not yet available)

#### P1.19 — Demo Recordings (HUMAN ONLY)
