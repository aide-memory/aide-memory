...
- **Fix: preserve user edits to rules files.** `aide-memory remember` (and any other memory write or config change) no longer rewrites the entire rules file out from under you.
  - `.claude/rules/aide-memory.md` is now static-only — left alone by all regen paths. It's only written by explicit `aide-memory init`, `init --update-rules`, or `init --force`. After init, the file is yours to edit.
  - `.cursor/rules/aide-memory.mdc` is rewritten on memory writes, but only from the `<!-- ─── AUTOGENERATED section ─── -->` marker onward. Everything above the marker (frontmatter + static teaching body + your custom additions) is preserved.
- New tests cover both behaviors: user edits to the Claude Code rules file survive a memory write; user edits before the autogen marker in the Cursor rules file survive too. If the marker has been removed, regen appends a fresh autogen block at the bottom rather than overwriting the file.
